...
gamma = 0.5; %a parameter in deciding the feasibility of join
    ...
    if joined
        ...
        while true
            ...
            if size(acttemp,2)~=0
                ...
                %moving agent 1 one step closer to the nearest target
                clt1 = cltar(G,agndtemp(1),acttemp);
                [path1, ~] = shortestpath(G,agndtemp(1),clt1);
                costtemp = costtemp + edgewt(eg, wt, agndtemp(1), path1(2));
                agndtemp(1) = path1(2);
                for i=1:size(acttemp,2)
                    if agndtemp(1) == acttemp(i)
                        acttemp(i) = [];
                        break
                    end
                end
...
%         end
        ...
        [path1, ~] = shortestpath(G, agnd(1),clt1);
        tot_cost = tot_cost + edgewt(eg, wt, agnd(1), path1(2));
        agnd(1) = path1(2);
        if agnd(1) == clt
                acttar(acttar == agnd(1)) = [];
        end
...
%         end
        ...
        %ag2pth = [ag2pth agnd(2)]
        %[spcost, ~, ~] = nntsp(G,agnd, acttar,1);
%         ag1pth = [ag1pth path1];
%         ag2pth = [ag2pth path2];
       % tot_cost = tot_cost + spcost;
...
%         end
        ...
        if agdis < gamma*ndcen_tar && splitbn
...
highlight(H,s1,t1,"EdgeColor","green")
highlight(H, s2, t2, "EdgeColor","blue")
